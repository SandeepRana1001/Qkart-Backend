...
const updateProductInCart = async (user, productId, quantity) => {
  ...
  for (let i = 0; i < cart.cartItems.length; i++) {
    if (productId == cart.cartItems[i].product._id) {
      productIndex = i;
      id = cart.cartItems[i].product._id;
    }
  }
...
const checkout = async (user) => {
  ...
  //first case
  if (cart === null) {
    throw new ApiError(httpStatus.NOT_FOUND, "User doesnot have a cart")
  }
  ...
  const totalInCart = cart.cartItems.reduce((acc, item) => {
    return acc + (parseInt(item.product.cost) * parseInt(item.quantity))
  }, 0)


  const walletMoney = user.walletMoney

  if (totalInCart > walletMoney) {
    throw new ApiError(
